...
RSpec.describe 'actions/upload_attachment', :vcr do
  ...
  describe 'execute' do
    # let(:attachmentStorageQuery) { JSON.parse(File.read('fixtures/actions/custom_operation/input/query_attachment.json')) }
    # let(:attachmentStorage) { connector.actions.custom_operation(attachmentStorageQuery)[:attachmentStorage] }
    let(:input) { 
      {
        'account' => 'wdc',
